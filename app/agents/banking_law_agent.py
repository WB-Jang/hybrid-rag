...
from tools.Tools import banking_law_search
...
# 은행법
class BankingRagState(CorrectiveRagState):
...
def retrieve_documents(state: BankingRagState) -> BankingRagState:
    print("---문서 검색---")
    query = state.get("rewritten_query", state["question"])
    docs = banking_law_search.invoke(query)
    return {"documents": docs}

def extract_and_evaluate_information(state: BankingRagState) -> BankingRagState:
    print("---정보 추출 및 평가---")
    extracted_strips = []

    for doc in state["documents"]:
        extract_prompt = ChatPromptTemplate.from_messages([
            ("system", """당신은 은행법 전문가입니다. 주어진 문서에서 질문과 관련된 주요 사실과 정보를 3~5개 정도 추출하세요. 
...
def rewrite_query(state: BankingRagState) -> BankingRagState:
    print("---쿼리 재작성---")

    rewrite_prompt = ChatPromptTemplate.from_messages([
        ("system", """당신은 은행법 전문가입니다. 주어진 원래 질문과 추출된 정보를 바탕으로, 더 관련성 있고 충실한 정보를 찾기 위해 검색 쿼리를 개선해주세요.
...
# def generate_node_answer(state: BankingRagState) -> BankingRagState:
#     print("---답변 생성---")
...
#     node_answer = gpt3_5_turbo_llm.invoke(answer_prompt.format(
#         question=state["question"],
#         extracted_info=extracted_info_str
#     ))
...
def should_continue(state: BankingRagState) -> Literal["계속", "종료"]:
...
# 그래프 생성
workflow = StateGraph(BankingRagState)
...
# 그래프 컴파일
banking_law_agent = workflow.compile()